...
**Exportación CSV completa (mismo día):** antes el único "Exportar CSV" era un resumen de KPIs del dashboard. Ahora hay 2 endpoints server-side (respetan filtros, BOM + RFC 4180, helper `apps/api/src/lib/csv.ts`): `GET /leads/export.csv` (prospectos con contacto, tipo, etapa, origen, propiedad+URL, UTM source/medium/campaign/content/term, asignado, consentimiento, mensaje) y `GET /properties/admin/export.csv` (inventario: tipo, operación, precios/monedas, m², rec/baños/estac., ubicación, estatus, destaque, remate, URL, fechas). Botón reutilizable `ExportCsvButton` en Leads, Propiedades y Dashboard (Leads/Inventario/Resumen). OpenAPI 52 rutas. Verificado en vivo. **Capturas móviles**: se instaló Playwright (Chromium; libs de sistema resueltas descargando .deb y extrayéndolas a LD_LIBRARY_PATH local, sin root) y se generó una galería-entregable (`scratchpad/gallery.html` → Artifact).
