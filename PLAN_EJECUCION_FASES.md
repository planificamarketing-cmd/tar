...
- [ ] Implementar esquema Drizzle completo (§4.1): users, refresh_tokens, locations, properties (con `geo` geography + índices GIN/GiST, estatus comercial, **precios duales venta/renta MXN-USD + normalizados**, `external_ref` EB, medios baños, piso, CP, tipos del inventario real), amenities, property_amenities, property_images, leads (`type` contacto/cita, `preferred_at`, `consent_at`), lead_events, webhook_subscriptions, webhook_deliveries, **api_keys**, **property_events**, marketing_scripts.
- [ ] Migración inicial + `db:seed` con datos de muestra tomados del CSV real (10 propiedades representativas: venta/renta, MXN/USD, los 7 tipos), amenidades derivadas de la columna `características`, 1 admin.
- [ ] Definir esquemas Zod compartidos en `packages/shared` para todas las entidades y filtros (§5).
- [ ] Publicar el **prototipo v3 en URL navegable** (hosting estático gratuito; es HTML autocontenido) para que TAR interactúe con él y entienda cómo puede quedar.
- [ ] Aplicar el rojo de marca definitivo **#D2103E** (✔ confirmado, tomado del logo) al prototipo antes de publicarlo.
- [ ] Sesión de revisión con TAR: capturar correcciones por escrito; aplicarlas al prototipo (hasta **3 rondas formales**, §13).
- [ ] **Firma del diseño** por TAR → la versión corregida del v3 se versiona en `design-reference/` como referencia obligatoria y se cierra la ventana de cambios UI sin costo.
- [ ] Tras la firma: portar tokens definitivos a `tailwind.config` (colores, Fraunces/Inter/DM Mono vía `next/font`).
- [ ] Generar borrador de ERD (`docs/ERD.pdf`) desde el esquema.
